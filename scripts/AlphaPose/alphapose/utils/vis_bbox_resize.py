        ...
        for n in range(kp_scores.shape[0]):
            ...
            if n < 26 or n > 93:
                if n < len(p_color):
                    ...
                    cv2.circle(bg, (int(cor_x), int(cor_y)), 4, (255,255,255), 2)
                ...
                if i < 24 or i > 83 :
                    if i < len(line_color):
                        if opt.tracking:
                            ...
                            cv2.line(bg, start_xy, end_xy, color, 2)
                        else:
                            cv2.line(bg, start_xy, end_xy, line_color[i], 2)
                            # cv2.fillConvexPoly(bg, polygon, line_color[i])
                    else:
                        cv2.line(bg, start_xy, end_xy, (255,255,255), 2)
                if n < len(p_color):
...
def vis_frame_bbox(frame, im_res, opt, name_image, format='coco'):
    ...
    height, width = img.shape[:2]
    incr = 0
    for human in im_res['result']: #Incrémentation sur le nombre de résultat donc le nombre de bbox
        part_line = {}
        kp_preds = human['keypoints']
        kp_scores = human['kp_score']
        if kp_num == 17:
        ...
        if (kp_scores[0] >= treshold_value and kp_scores[-1] >= treshold_value ):
            ...
            new_image = []
            new_image = img[ position[1]:position[3] , position[0]:position[2] , 0:3]
            os.makedirs(opt.outputpath + 'bbox/crop/', exist_ok=True)
            ...
            if int((new_image.shape[0]*100)/img.shape[0]) > opt.size_bbox or int((new_image.shape[1]*100)/img.shape[1]) > opt.size_bbox:
                cv2.imwrite(opt.outputpath + 'bbox/crop/' + new_name,new_image)
                incr+=1
