...
class DataWriter():
                    ...
                    _result.append(
                        {
                            ...
                            'kp_score':preds_scores[k],# if add *5 increase size of the line to draw for body
    ...
    def write_image(self, img, im_name, stream=None):
        ...
        if self.opt.save_img:
            ...
            path = os.path.join(self.opt.outputpath, 'bbox' ,'vis_crop')
            os.makedirs(path, exist_ok=True)
            cv2.imwrite(os.path.join(self.opt.outputpath, 'bbox' ,'vis_crop', im_name), img)
        if self.save_video:
            stream.write(img)
